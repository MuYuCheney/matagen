...
def extract_github_repos(search_results):
    # 使用列表推导式筛选出项目主页链接
    repo_links = [result['link'] for result in search_results if
                  '/issues/' not in result['link'] and '/blob/' not in result['link'] and 'github.com' in result[
                      'link'] and len(result['link'].split('/')) == 5]
...
def internet_config_assistant():
    user_input = input(
        "你好，欢迎MateGen互联网配置助手，请输入你希望获取的帮助类型：1.获取帮助文档；2.进行互联网参数配置；0.退出")
    if user_input == '1':
...
def write_oss_env(access_key_id, access_key_secret, endpoint, bucket_name):
    ...
    if load_dotenv(dotenv_path):
        ...
        print(
            f".env 文件已更新: OSS_ACCESS_KEY_ID={access_key_id}, OSS_ACCESS_KEY_SECRET={access_key_secret}, OSS_ENDPOINT={endpoint}, OSS_BUCKET_NAME={bucket_name}")
    else:
        print("找不到配置文件，请先编写配置文件")
        return None
    ...
    if api_key is None or cse_id is None:
        ...
        try:
            ...
            response = requests.get(PROXY_URL,
                                    params={'api_key': api_key, 'cse_id': cse_id, 'q': query, 'num_results': 1})
            if response.status_code == 200:
                return True
            else:
                return False
...
def muyu_main():
    assistant = MateGenClass(
        api_key="gAAAAABmtbNCz6ZpAniNVLTaq506_U2x5ark89pPaKmKEqkEC4RBxJDTNjZwccJgmZm5sL9bXiW1Oi7FExBSgsEv89-NsgLfn6NqBQmOnF2Ln6QLRgzqoUsVQU7Hz9VibOCOpDkvImXVO7yPuNBkYCnafdzPO1N-0A==")
    print("成功实例化MateGen Agent")
    assistant.chat()
