...
def mount_app_routes(app: FastAPI):
    ...
    def check_database_initialization():
        ...
        try:
            if check_and_initialize_db(db_session, "13001246660") == '':
                return {"status": 400,
    ...
    def save_api_key(api_key: str = Body(..., description="用于问答的密钥", embed=True),
        ...
        try:
            ...
            # 获取解密后的真实API_KEY用于数据库存储
            api_key, angent_type = get_key_info(api_key)
            # 存储用户加密后的 API_KEY
            if upsert_agent_by_user_id(db_session, api_key, user_id='13001246660'):
    ...
    async def chat(query: str = Query(..., description="用户会话框输入的问题"),
                   thread_id: bool = Query(True, description="会话id"),
        ...
        try:
            # 使用SSE 流式处理
            event_response = EventSourceResponse(event_generator(thread_id, query, code_type, run_result))
    ...
    async def event_generator(thread_id, question, code_type, run_result):
        mategen_instance = MateGenClass()
        client = mategen_instance.cache_pool.get_async_client(user_id="13001246660")

        assis_id = mategen_instance.chat(question, chat_stream=True)
        db_session = SessionLocal()
        ...
        if code_type != None:
            ...
            if mategen_instance.knowledge_base_name_id:
                kb_info = db_session.query(KnowledgeBase).filter(
                    KnowledgeBase.id == mategen_instance.knowledge_base_name_id).one_or_none()
                if kb_info:
                    knowledge_name = kb_info.display_knowledge_base_name

            # 检查是否存在 db_name_id
            if mategen_instance.db_name_id:
                db_info = db_session.query(DbBase).filter(
                    DbBase.id == mategen_instance.db_name_id).one_or_none()
                if db_info:
            ...
            new_message = MessageModel(
                ...
                knowledge_id=mategen_instance.knowledge_base_name_id if mategen_instance.knowledge_base_name_id else None,
                knowledge_name=knowledge_name,
                db_id=mategen_instance.db_name_id if mategen_instance.db_name_id else None,
            ...
            async with client.beta.threads.runs.stream(
                ...
                if full_text == '':
                    text = await client.beta.threads.messages.list(thread_id=thread_id)
    ...
    async def upload_files(
        ...
        mategen_instance = MateGenClass()
        client = mategen_instance.cache_pool.get_client(user_id="13001246660")
...
def run_api(host, port, **kwargs):
    ...
    from db.base_model import initialize_database
